...
class BasicConv2d(nn.Module):           #基础卷积块：CONV2d+BN+RELU
...
class aggregation(nn.Module):       #聚合特征
...
class Inf_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x2_rfb = self.rfb2_1(x2)        # channel -> 32         f3
        x3_rfb = self.rfb3_1(x3)        # channel -> 32         f4
        x4_rfb = self.rfb4_1(x4)        # channel -> 32         f5

        # ---- edge guidance ----        边界指导
        x = self.edge_conv1(x1)
        x = self.edge_conv2(x)
        edge_guidance = self.edge_conv3(x)  # torch.Size([1, 64, 88, 88])   #[1,64,176,176]
        lateral_edge = self.edge_conv4(edge_guidance)   # NOTES: Sup-2 (bs, 1, 88, 88) -> (bs, 1, 352, 352)
        lateral_edge = F.interpolate(lateral_edge,          #改变大小
                                     scale_factor=2,
                                     mode='bilinear')

        # ---- global guidance ----       全局指导
        ra5_feat = self.ParDec(x4_rfb, x3_rfb, x2_rfb)    #Sg  88
        lateral_map_5 = F.interpolate(ra5_feat,
                                      scale_factor=4,
                                      mode='bilinear')    # NOTES: Sup-1 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        # ---- reverse attention branch_4 ----
        crop_4 = F.interpolate(ra5_feat, scale_factor=0.25, mode='bilinear')  #  22
        x = -1*(torch.sigmoid(crop_4)) + 1  # reverse    #反相RA
        ...
        crop_2 = F.interpolate(x, scale_factor=2, mode='bilinear')  #88
